...
def test_leading_dot_slash_alias_is_caught_as_a_collision(tmp_path):
    """`./services/api/app/auth.py` and `services/api/app/auth.py` name the same file."""
    body = VALID.replace(
        "- Create: `app/one.py`", "- Modify: `services/api/app/auth.py`"
    ).replace("- Create: `app/two.py`", "- Modify: `./services/api/app/auth.py`")
    r = run_script("plan-tasks", "--validate", str(_plan(tmp_path, body)), cwd=tmp_path)
    assert r.returncode == 1
    assert "wave 1" in r.stderr
    assert "services/api/app/auth.py" in r.stderr
