...
- **No native tool? Fall back to git — and always name the base ref
  ...
  - Pick `<path>` inside the project's configured `worktree_dir` at the
    project root — its established worktree home. Before creating anything
    there, confirm it's git-ignored (`git check-ignore -q "<worktree_dir>"`);
    if it isn't, add it to `.gitignore` and commit that first, or the whole
    worktree's contents get committed into the repo.
